...
__IO uint32_t  DCMI_TIMEOUT_MAX = SCCB_Open207Z_FLAG_TIMEOUT;
...
void SCCB_GPIO_Config(void)
{
	...
	RCC_AHB1PeriphClockCmd(Open207Z_SCCB_SDA_GPIO_CLK | Open207Z_SCCB_SCL_GPIO_CLK,ENABLE);
	
	RCC_APB1PeriphClockCmd(Open207Z_SCCB_CLK,ENABLE);
	
	GPIO_PinAFConfig(Open207Z_SCCB_SDA_GPIO_PORT, Open207Z_SCCB_SDA_SOURCE, Open207Z_SCCB_SDA_AF);
	GPIO_PinAFConfig(Open207Z_SCCB_SCL_GPIO_PORT, Open207Z_SCCB_SCL_SOURCE, Open207Z_SCCB_SCL_AF);
	
	GPIO_InitStructure.GPIO_Pin =  Open207Z_SCCB_SDA_PIN | Open207Z_SCCB_SCL_PIN;
	...
	I2C_Cmd(Open207Z_SCCB, ENABLE);
	I2C_Init(Open207Z_SCCB, &SCCB_InitStructure);
	
	I2C_AcknowledgeConfig(Open207Z_SCCB, ENABLE);	
}
...
/**
  ...
  */
uint8_t DCMI_SingleRandomWrite_(uint8_t Reg, uint8_t Data)
{
  ...
  I2C_Send7bitAddress(Open207Z_SCCB, OV7670_DEVICE_WRITE_ADDRESS, I2C_Direction_Transmitter);
  ...
  while(!I2C_CheckEvent(Open207Z_SCCB, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
  {
   ...
   /*-----------------------------------------------------------------------------------*/
  /* Send Data */
  I2C_SendData(Open207Z_SCCB, Data);    
...
/**
  ...
  *         occured. 
  */
uint8_t DCMI_SingleRandomRead_(uint8_t Reg, uint8_t *Data)
{
  uint32_t timeout = DCMI_TIMEOUT_MAX;
