...
void MCO1_Init(void)
{
	...
	RCC_MCO1Config(RCC_MCO1Source_HSE, RCC_MCO1Source_HSE);
}
...
uint8_t DCMI_OV7670_Init(void)
{
	...
	//DCMI_0V7670_RST_PWDN_Init();
  	...
  	Delay_ms(0xfff);
	if(DCMI_SingleRandomWrite_(OV7670_COM7, SCCB_REG_RESET)!=0)
		return 0xff;
	Delay_ms(0xfff);
  
	for(i=0;i<CHANGE_REG_NUM;i++)
	{
		if(DCMI_SingleRandomWrite_(change_reg[i][0],change_reg[i][1])!=0)
		{
			return 0xff;
		}
	}
...
{
	uint8_t temp;
	if(DCMI_SingleRandomRead_(OV7670_MIDH,&temp)!=0)
		return 0xff;
	OV7670ID->Manufacturer_ID1 = temp;
	if(DCMI_SingleRandomRead_(OV7670_MIDL,&temp)!=0)
		return 0xff;
	OV7670ID->Manufacturer_ID2 = temp;
	if(DCMI_SingleRandomRead_(OV7670_VER,&temp)!=0)
		return 0xff;
	OV7670ID->Version = temp;
	if(DCMI_SingleRandomRead_(OV7670_PID,&temp)!=0)
		return 0xff;
	OV7670ID->PID = temp;
...
void OV7670_config_window(uint16_t startx, uint16_t starty, uint16_t width, uint16_t height)
{
	...
	state = DCMI_SingleRandomRead_(0x03, &temp_reg1 );
	temp_reg1 &= 0xC0;
	state = DCMI_SingleRandomRead_(0x32, &temp_reg2 );
	temp_reg2 &= 0xC0;
	
	// Horizontal
	temp = temp_reg2|((endx&0x7)<<3)|(startx&0x7);
	state = DCMI_SingleRandomWrite_(0x32, temp );
	temp = (startx&0x7F8)>>3;
	state = DCMI_SingleRandomWrite_(0x17, temp );
	temp = (endx&0x7F8)>>3;
	state = DCMI_SingleRandomWrite_(0x18, temp );
	
	// Vertical
	temp = temp_reg1|((endy&0x7)<<3)|(starty&0x7);
	state = DCMI_SingleRandomWrite_(0x03, temp );
	temp = (starty&0x7F8)>>3;
	state = DCMI_SingleRandomWrite_(0x19, temp );
	temp = (endy&0x7F8)>>3;
	state = DCMI_SingleRandomWrite_(0x1A, temp );
}
