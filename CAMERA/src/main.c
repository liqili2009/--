...
#define FSMC_LCD_ADDRESS    0x68000002
 uint8_t testflag=0;
 uint8_t testdata[0xd0];
 uint8_t testi=0;
 uint8_t testj=0;
 OV9655_IDTypeDef idnum;
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
int main(void)
{
	...
	STM_EVAL_LEDOn(LED1);
	IOE_Config();
	IOE_WriteIOPin(CAMERA_PLUG_PIN, BitReset);
	...
	{
		/*    for(testi=0;testi<0xff;testi++){
		testdata[testi]=DCMI_SingleRandomRead(OV9655_DEVICE_READ_ADDRESS,testi);
		//testdata[1]=DCMI_SingleRandomRead(OV9655_DEVICE_READ_ADDRESS,0x0b);
		Delay(10);
		}  */
		DCMI_OV9655_ReadID(&idnum);
		...
		while(1)
		{
			...
			if(OV9655PCLKOutput < 0x4c)
			{
				...
				testflag=DCMI_SingleRandomWrite(OV9655_DEVICE_WRITE_ADDRESS,OV9655_TSLB, OV9655PCLKOutput);
				if( testflag==0xff){
					STM_EVAL_LEDOn(LED3);
					//while(1);
				}
