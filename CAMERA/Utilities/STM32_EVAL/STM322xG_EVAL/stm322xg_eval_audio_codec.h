...
/* Includes ------------------------------------------------------------------*/
#include "stm32f2xx.h"
#include "stm322xg_eval_ioe_uda1380.h" /* IOExpander driver is included in order to allow 
...
/*typedef union 
{  
  unsigned char read_codec_reg8[2];
  unsigned short read_codec_reg16;
} ReadCodeReg_TypeDef; */
/*------------------------------------
...
/* Headphone driver and interpolation filter(Register map of status bits (read-out))			  */
#define UDA1380_REG_FILTER_STATUS       (0x18)
#define UDA1380_REG_DECIMATOR_STATUS    (0x28)
/**
  * @}
  */ 
...
/** @defgroup STM322XG_EVAL_AUDIO_CODEC_Exported_Macros
  * @{
  */ 
#define VOLUME_CONVERT(x)    ((Volume > 100)? 100:((uint8_t)(252-((Volume * 252) / 100))))
