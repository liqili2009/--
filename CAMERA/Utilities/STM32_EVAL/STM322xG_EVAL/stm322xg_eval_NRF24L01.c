...
{		
	...
	SPI_I2S_SendData(SPI3, TxData);                                    //通过外设SPI3发送一个数据
	retry=0;
	...
	return SPI_I2S_ReceiveData(SPI3);                                 //通过SPI3返回接收数据				    
}
//NRF24L01 驱动函数 
...
u8 NRF24L01_Write_Reg(u8 regaddr,u8 data)
{
    ...
    Clr_NRF24L01_CSN;                    //使能SPI传输
  	status =SPI3_ReadWriteByte(regaddr); //发送寄存器号 
  	SPI3_ReadWriteByte(data);            //写入寄存器的值
...
u8 NRF24L01_Read_Reg(u8 regaddr)
{
	u8 reg_val;	    
 	Clr_NRF24L01_CSN;                //使能SPI传输		
  	SPI3_ReadWriteByte(regaddr);     //发送寄存器号
...
u8 NRF24L01_Read_Buf(u8 regaddr,u8 *pBuf,u8 datalen)
{
	u8 status,u8_ctr;	       
  	Clr_NRF24L01_CSN;                     //使能SPI传输
  	status=SPI3_ReadWriteByte(regaddr);   //发送寄存器值(位置),并读取状态值   	   
 	for(u8_ctr=0;u8_ctr<datalen;u8_ctr++)pBuf[u8_ctr]=SPI3_ReadWriteByte(0XFF);//读出数据
...
u8 NRF24L01_Write_Buf(u8 regaddr, u8 *pBuf, u8 datalen)
{
	u8 status,u8_ctr;	    
 	Clr_NRF24L01_CSN;                                    //使能SPI传输
  	status = SPI3_ReadWriteByte(regaddr);                //发送寄存器值(位置),并读取状态值
  	for(u8_ctr=0; u8_ctr<datalen; u8_ctr++)SPI3_ReadWriteByte(*pBuf++); //写入数据	 
  	Set_NRF24L01_CSN;                                    //关闭SPI传输
  	return status;                                       //返回读到的状态值
}				   
//启动NRF24L01发送一次数据
