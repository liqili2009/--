...
/** @addtogroup STM322XG_EVAL_AUDIO_CODEC
  * @brief       This file includes the low layer driver for UDA1380 Audio Codec
  *              available on STM322XG-EVAL evaluation board(MB786).
  * @{
  */ 
...
#define CODEC_ADDRESS                   0x30  /* b00110A0R/W */
/**
  * @}
  */ 
...
__IO uint8_t OutputDev = OUTPUT_DEVICE_BOTH;
...
/*========================
                ...
                UDA1380 Audio Codec Control Functions
                                                ==============================*/
/**
...
//  uint32_t counter = 0; 
  ...
  ////////////////////////////////////////////
  /* software reset  */
  testcounter += Codec_WriteRegister(UDA1380_REG_RESET, 0x0000);
  /* Keep Codec powered OFF */
  testcounter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, 0x0000); /*1010 0101 0000 0000 */
  /* Power on the Codec */
  //testcounter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL,0xa5df); //1010 0101 0000 0000
											                   //1010 0101 1101 1111

  /* Clock configuration:  */  
  //testcounter += Codec_WriteRegister(UDA1380_REG_CLOCKS, 0x0f39);	//0000 0111 0000 0010
  														     	//0000 1111	0011 0010
  /* 2S-bus input and output settings  */
  testcounter += Codec_WriteRegister(UDA1380_REG_I2S, 0x0000);	//0000 0000 0001 0000  

  /* Analog mixer settings  */
  //testcounter += Codec_WriteRegister(UDA1380_REG_ANALOG, 0x0000);	//0010 0010 0010 0010  

  /* Set the Master volume */
  testcounter += Codec_VolumeCtrl(Volume);

  /* mixer volume control */
  testcounter += Codec_WriteRegister(UDA1380_REG_MIXER_VOL, 0x0000);

  /* Adjust Bass and Treble levels */
  //testcounter += Codec_WriteRegister(UDA1380_REG_MODE_SEL, 0x5505);

  /* master mute, channel 1 and channel 2 de-emphasis and channel mute */
  testcounter += Codec_WriteRegister(UDA1380_REG_MASTER_MUTE, 0x0000);

  /* mixer, silence detector and interpolation filter oversampling settings */
  testcounter += Codec_WriteRegister(UDA1380_REG_MIXER_SET, 0x0000);
 
    /* decimator volume control */
  //testcounter += Codec_WriteRegister(UDA1380_REG_DEC_VOL, 0x0000);

  /* PGA settings and mute */
  //testcounter += Codec_WriteRegister(UDA1380_REG_PGA_SET, 0x0000);

    /* ADC settings */
  //testcounter += Codec_WriteRegister(UDA1380_REG_ADC_SET, 0x0000);

  /* AGC settings */
  //testcounter += Codec_WriteRegister(UDA1380_REG_AGC_SET, 0x0000);
  ...
  //Codec_AudioInterface_Init(AudioFreq);  
 	 	////////////////////////////////////////////
	counter = 0;
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_CLOCKS);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_I2S);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_PWR_CTRL);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_ANALOG);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_HPHONE);

	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_MASTER_VOL);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_MIXER_VOL);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_MODE_SEL);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_MASTER_MUTE);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_MIXER_SET);

	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_DEC_VOL);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_PGA_SET);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_ADC_SET);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_AGC_SET);

	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_RESET);

	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_FILTER_STATUS);
	read_codec_reg16[counter++] = Codec_ReadRegister(UDA1380_REG_DECIMATOR_STATUS);
...
static uint32_t Codec_PauseResume(uint32_t Cmd)
{
  ...
  { 
    ...
    counter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, 0x0000);    
  }
  else /* AUDIO_RESUME */
  {
    ...
    counter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, 0xa5df); 
  }
...
static uint32_t Codec_Stop(uint32_t CodecPdwnMode)
{
  ...
  {    
    ...
    counter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, 0x2400);
  }
  else /* CODEC_PDWN_HW */
  { 
    /* Power down the DAC and the speaker (PMDAC and PMSPK bits)*/
...
{
  uint32_t counter = 0;
  uint16_t VolLR=Volume+((uint16_t)Volume<<8);

  /* Set the Master volume */
  counter += Codec_WriteRegister(UDA1380_REG_MASTER_VOL, VolLR); 
...
static uint32_t Codec_Mute(uint32_t Cmd)
{
  ...
  {
    counter += Codec_WriteRegister(UDA1380_REG_MASTER_MUTE, 0x4808);
  }
  else /* AUDIO_MUTE_OFF Disable the Mute */
  {
    counter += Codec_WriteRegister(UDA1380_REG_MASTER_MUTE, 0x0000);
  }
...
static void Codec_Reset(void)
{
  ...
  /* Power on the codec */
   IOE_WriteIOPin(IOE_2_ADDR, AUDIO_RESET_PIN, BitReset);
}
...
uint32_t Codec_SwitchOutput(uint8_t Output)
{
  ...
  output_dev = Codec_ReadRegister(UDA1380_REG_PWR_CTRL);
  switch (Output) 
  {	      
    case OUTPUT_DEVICE_SPEAKER:
	  output_dev &= ~0x2400;
      counter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, output_dev|0x0400); /* SPK always ON & HP always OFF */
      OutputDev = OUTPUT_DEVICE_SPEAKER;
      break;
      
    case OUTPUT_DEVICE_HEADPHONE:
	  output_dev &= ~0x2400;
      counter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, output_dev|0x2000); /* SPK always OFF & HP always ON */
      OutputDev = OUTPUT_DEVICE_HEADPHONE;
      break;
      
    case OUTPUT_DEVICE_BOTH:
      counter += Codec_WriteRegister(UDA1380_REG_PWR_CTRL, output_dev|0x2400); /* SPK always ON & HP always ON */
      OutputDev = OUTPUT_DEVICE_BOTH;
      break;     
...
//  uint32_t result = 0;
  ...
  I2C_SendData(CODEC_I2C, (RegisterValue>>8));
  ...
  /* Prepare the register value to be sent */
  I2C_SendData(CODEC_I2C, RegisterValue&0xff);
  ...
  testresult = Codec_ReadRegister(RegisterAddr);
  Gresult = (testresult == RegisterValue)? 0:1;
...
static uint32_t Codec_ReadRegister(uint8_t RegisterAddr)
{
  ...
  /*configures the same bit (POS=1)*/
  I2C_NACKPositionConfig(CODEC_I2C, I2C_NACKPosition_Next);

  /* Clear ADDR register by reading SR1 then SR2 register (SR1 has already been read) */
  (void)CODEC_I2C->SR2;	  //Clear ADDR

  /*!< Disable Acknowledgment */
  I2C_AcknowledgeConfig(CODEC_I2C, DISABLE); //ACK=0  
  
  /* Wait on BTF flag to be set  */
  CODECTimeout = CODEC_FLAG_TIMEOUT;
  while(I2C_GetFlagStatus(CODEC_I2C, I2C_FLAG_BTF) == RESET)
  {
    if((CODECTimeout--) == 0) return Codec_TIMEOUT_UserCallback();
  ...
  /*!< Send STOP Condition */
  I2C_GenerateSTOP(CODEC_I2C, ENABLE);	//STOP=1
  
  /*!< Read the first byte received from the Codec */
  result = I2C_ReceiveData(CODEC_I2C);			   //Read the data
  result<<=8;
  /*!< Read the second byte received from the Codec */
  result += I2C_ReceiveData(CODEC_I2C);			   //Read the data

  /* Wait to make sure that STOP flag has been cleared */
  CODECTimeout = CODEC_FLAG_TIMEOUT;			  //Wait until STOP is cleared by hardware
  while(CODEC_I2C->CR1 & I2C_CR1_STOP)
  {
    if((CODECTimeout--) == 0) return Codec_TIMEOUT_UserCallback();
  } 

   /*configures the same bit (POS)*/
  I2C_NACKPositionConfig(CODEC_I2C, I2C_NACKPosition_Current);

  /*!< Re-Enable Acknowledgment to be ready for another reception */
  I2C_AcknowledgeConfig(CODEC_I2C, ENABLE);  	 //ACK = 1 (to be ready for	another reception).
...
#if 0
static uint32_t Codec_ReadRegister1(uint8_t RegisterAddr)
{
  uint32_t result = 0;
  ...
  CODECTimeout = CODEC_FLAG_TIMEOUT;
  while(I2C_GetFlagStatus(CODEC_I2C, I2C_FLAG_RXNE) == RESET)	 //Wait until RXNE = 1
  {
    if((CODECTimeout--) == 0) return Codec_TIMEOUT_UserCallback();
  }
