...
void LCD_L0_DrawHLine  (int x0, int y,  int x1)
{
	Lcd_Start();
	Lcd_SetCursor(x0,y );
	LCD_WR_REG(0x0052,x0);//水平GRAM起始位置
	LCD_WR_REG(0x0053,x1);//水平GRAM终止位置
	LCD_WR_REG(0x0050,y);//垂直GRAM起始位置
	LCD_WR_REG(0x0051,y);//垂直GRAM终止位置 
	Lcd_WR_Start();
	x1=x1-x0;  	
	for (x0 = 0; x0 <= x1; x0++)
	{
		DataToWrite(LCD_COLORINDEX);
		Clr_nWr;
		Set_nWr;
	}
	Lcd_Start();
...
{
	Lcd_SetCursor(x, y0); 
	LCD_WR_REG(0x0052,x);//水平GRAM起始位置
	LCD_WR_REG(0x0053,x);//水平GRAM终止位置
	LCD_WR_REG(0x0050,y0);//垂直GRAM起始位置
	LCD_WR_REG(0x0051,y1);//垂直GRAM终止位置
	Lcd_WR_Start();
	y1=y1-y0;
	for (y0 = 0; y0 <= y1; y0++)
	{
